...
You build Go in someone else's repository against the brief below. One checkout, one step.
...
`build`, `test`, `lint` and `fmt`. Where there is none, use the raw commands below. A module in a subfolder is
reached raw with `go -C <folder> …`, since you cannot `cd`.
