...
export const THEN = ['return', 'retry', 'done', 'park', 'ticket', 'ask_ceo'] as const
...
export async function fixer(db: Db, root: string, plan: PlanRow, decision: { id: number; why: string }, ticket: string,
  ...
  const tokens = fired.usage.input + fired.usage.output
  const got = fired.exit === 0 ? read(fired.text) : null
  if (got === null) {
